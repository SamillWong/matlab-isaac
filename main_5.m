% ============================================================
% samiko // 2020-04-25
% MATLAB IMAGE TO ASCII ART CONVERTER Version 0.5
% ============================================================
...
try
    ...
    % Apply new trimmed dimensions to avoid conflicting array sizes
...
try
    ...
    if size(output_data,1)*size(output_data,2) >= 532000
        errorStruct.identifier = 'SAVEFILE:OutputTooLarge';
        error(errorStruct);
    end
    
    % Checks if output column count is too large
    if size(output_data,2) >= 1080
        errorStruct.identifier = 'SAVEFILE:OutOfRange';
        error(errorStruct);
    end
    ...
    % Exception handler
    switch e.identifier
        case 'SAVEFILE:OutputTooLarge'
            errorStruct.message = "[x] ERROR: Output is too large. Please try a smaller image.";
            error(errorStruct);
        case 'SAVEFILE:OutOfRange'
            errorStruct.message = "[x] ERROR: Failed to save file. Please try a smaller output size.";
            error(errorStruct);
